...
final class ExerciseDBCatalogService {
    ...
    func synchronize(
        ...
        if force {
            ...
            if try catalogNeedsMediaRepair(in: context) {
                resetCheckpoint()
            } else {
                _ = try ExerciseCatalogDeduplicationService.reconcile(in: context)
                defaults.set(
                    try catalogRecordCount(in: context),
                    forKey: CheckpointKey.storedRecordCount
                )
        ...
        do {
            while true {
                ...
                if page.meta.hasNextPage, let lastID = page.data.last?.exerciseId {
                    defaults.set(lastID, forKey: CheckpointKey.nextCursor)
                } else {
                ...
                guard let lastID = page.data.last?.exerciseId,
                      !lastID.isEmpty,
                      cursorsSeen.insert(lastID).inserted
                else {
                    throw ExerciseCatalogError.stalledPagination
                }
                cursor = lastID
    ...
    func searchAndImport(query: String, in context: ModelContext) async throws -> Int {
        let query = ExerciseNameNormalizer.normalize(query)
        ...
        guard page.success else { throw ExerciseCatalogError.unsuccessfulResponse }
        let result = try merge(page.data, into: context)
        _ = try ExerciseCatalogDeduplicationService.reconcile(in: context, saveChanges: false)
        try context.save()
    ...
    func enrichIfNeeded(_ exercise: Exercise, in context: ModelContext) async throws -> Bool {
        ...
        var candidates: [ExerciseDBExerciseDTO] = []
        if let override = ExerciseSeedMediaOverrides.override(for: exercise) {
            var seen = Set<String>()
            for query in ([exercise.name] + override.enrichSearchQueries) {
                for remote in try await searchCandidates(for: query, maximumPages: 4) {
                    guard seen.insert(remote.exerciseId).inserted else { continue }
                    candidates.append(remote)
                }
            }
            if let preferred = candidates.first(where: { $0.exerciseId == override.catalogExerciseID }) {
                return try applyEnrichmentMatch(preferred, to: exercise, in: context)
            }
            if validatedMediaURLString(override.mediaURLString) != nil {
                let stub = ExerciseDBExerciseDTO(
                    exerciseId: override.catalogExerciseID,
                    name: exercise.name,
                    gifUrl: override.mediaURLString,
                    bodyParts: [],
                    equipments: [],
                    targetMuscles: [],
                    secondaryMuscles: [],
                ...
                return try applyEnrichmentMatch(stub, to: exercise, in: context)
            }
        } else if ExerciseSeedService.activeSeedNormalizedNames.contains(
            ExerciseNameNormalizer.normalize(exercise.name)
        ) {
            // Curated names without a hand-verified match intentionally keep the
            // placeholder. A fuzzy match can depict a different grip or machine.
            return false
        } else {
            candidates = try await searchCandidates(for: exercise.name, maximumPages: 8)
        }

        guard let match = bestCandidate(for: exercise, from: candidates) else { return false }
        return try applyEnrichmentMatch(match, to: exercise, in: context)
    }

    private func applyEnrichmentMatch(
        _ match: ExerciseDBExerciseDTO,
        ...
        if let owner = allExercises.first(where: { $0.externalCatalogID == match.exerciseId }),
           owner.id != exercise.id {
            guard ExerciseSeedMediaOverrides.override(for: exercise)?.catalogExerciseID == match.exerciseId else {
                return false
            }
        }
        ...
        if didChange || deduplication.didChange { try context.save() }
        return didChange || deduplication.didChange
    }
    ...
    /// Applies verified seed media overrides after catalog sync.
    @discardableResult
    func enrichSeedExercisesIfNeeded(in context: ModelContext) async throws -> Int {
        let exercises = try context.fetch(FetchDescriptor<Exercise>())
        var enriched = 0
        for exercise in exercises where exercise.mediaURLString?.isEmpty != false {
            guard ExerciseSeedMediaOverrides.override(for: exercise) != nil else { continue }
            if try await enrichIfNeeded(exercise, in: context) {
                enriched += 1
            }
        }
        return enriched
    ...
    func backfillMissingMediaLocally(in context: ModelContext) throws -> Int {
        ...
        let donors: [(tokens: Set<String>, exercise: Exercise)] = all.compactMap { exercise in
            ...
            let tokens = localSearchTokens(for: exercise)
        ...
        for recipient in all where recipient.mediaURLString?.isEmpty != false {
            let wantedTokens = Set(
                normalizedSearchText(recipient.name).split(separator: " ").map(String.init)
            )
            guard !wantedTokens.isEmpty else { continue }

            let candidates = donors.filter { donor in
                guard donor.exercise.id != recipient.id,
                      donor.exercise.equipment == recipient.equipment,
                      donor.exercise.primaryMuscleGroup == recipient.primaryMuscleGroup
                        || donor.exercise.secondaryMuscleGroups.contains(recipient.primaryMuscleGroup)
                else { return false }
                return wantedTokens.isSubset(of: donor.tokens)
            }
    ...
    private func localSearchTokens(for exercise: Exercise) -> Set<String> {
        Set(
            ([exercise.name] + exercise.searchAliases)
                .flatMap { normalizedSearchText($0).split(separator: " ").map(String.init) }
        )
    }
    ...
    func searchCandidates(for name: String, maximumPages: Int) async throws -> [ExerciseDBExerciseDTO] {
        ...
        for _ in 0..<maximumPages {
            ...
            guard page.meta.hasNextPage,
                  let lastID = page.data.last?.exerciseId,
                  !lastID.isEmpty,
                  seen.insert(lastID).inserted
            else { break }
            cursor = lastID
        }
        return matches
...
private extension ExerciseDBCatalogService {
    ...
    func merge(_ remoteExercises: [ExerciseDBExerciseDTO], into context: ModelContext) throws -> MergeResult {
        ...
        for exercise in existing {
            if let externalID = exercise.externalCatalogID, !externalID.isEmpty {
                if byExternalID[externalID] == nil || byExternalID[externalID]?.isArchived == true {
                    byExternalID[externalID] = exercise
                }
            }
            let normalizedName = ExerciseNameNormalizer.normalize(exercise.name)
            if !exercise.isCustom,
               byNormalizedName[normalizedName] == nil || byNormalizedName[normalizedName]?.isArchived == true {
                byNormalizedName[normalizedName] = exercise
            }
        }
        // Curated seed names own their verified provider ID. This lets a sync enrich
        // the short, familiar name instead of inserting a second API-named row.
        for exercise in existing where !exercise.isCustom && !exercise.isArchived {
            guard let override = ExerciseSeedMediaOverrides.override(for: exercise) else { continue }
            byExternalID[override.catalogExerciseID] = exercise
        }
    ...
    func enrich(
        ...
        if exercise.popularityRank == ExercisePopularity.unrankedRank,
           let rank = ExercisePopularity.rank(
               forNormalizedName: ExerciseNameNormalizer.normalize(exercise.name)
           ) {
    ...
    /// Re-sync when a completed checkpoint no longer has movement media attached.
    func catalogNeedsMediaRepair(in context: ModelContext) throws -> Bool {
        guard defaults.bool(forKey: CheckpointKey.isComplete) else { return false }
        let exercises = try context.fetch(FetchDescriptor<Exercise>())
        guard exercises.count >= 50 else { return false }
        let withMedia = exercises.filter { !($0.mediaURLString ?? "").isEmpty }.count
        return withMedia < exercises.count / 2
    }
...
private extension ExerciseDBCatalogService {
    ...
    func aliases(for remote: ExerciseDBExerciseDTO, mapped: MappedClassification) -> [String] {
        var aliases = remote.bodyParts
            ...
            + movementAliases(for: remote.name)
    ...
    func movementAliases(for name: String) -> [String] {
        switch normalizedSearchText(name) {
        case "dumbbell bent over row":
            ["Dumbbell Bent-Over Row", "Bent-Over Dumbbell Row", "Two-Dumbbell Row"]
        case "cable hammer curl with rope", "cable hammer curls rope attachment":
            ["Rope Hammer Curl", "Cable Rope Hammer Curl"]
        case "cable curl", "cable close grip curl":
            ["Cable Biceps Curl", "Cable Bicep Curl", "Straight-Bar Cable Curl"]
        default:
            []
        }
    }
